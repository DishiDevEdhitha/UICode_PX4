...
def capture_image(l_path_images):
    ...
    try:
        ...
        capture_command = (f"ffmpeg -f v4l2 -framerate 30 -video_size 1920x1080 -input_format mjpeg " f"-i /dev/video0 -vf 'fps=1' -q:v 2 {l_path_images}/image_%05d.jpg")
            ...
            if msg.get_type() == 'GLOBAL_POSITION_INT':
                ...
                if heading_en:
                    gps_entry = (
                        ...
                        ((msg.hdg / 100) + cam_z_rotation) % 360,  # Heading
                        ((msg.hdg / 100) + cam_z_rotation) % 360,  # Yaw (same as heading)
                        timestamp
                    )
                else:
                    gps_entry = (
                        msg.lat / 1e7, msg.lon / 1e7, msg.relative_alt / 1e3,
                        ((msg.hdg / 100) + cam_z_rotation) % 360,
...
def newton_interpolation(points, x):
    ...
    divided_diffs = [points[i][0] for i in range(n)]  # Use only the first value (latitude, longitude, etc.)
...
'''
    ...
    while len(mavlink_conn) == 0:
        ...
        if using_wire_connection:
            ...
            print(blue_text, 'Check UDP port: 127.0.0.1:14569', reset_color)
        else:
            print(blue_text, 'Check TCP port: 127.0.0.1:5760', reset_color)
